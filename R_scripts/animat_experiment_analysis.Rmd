...
```{r}
mdl_5 <- stan_glmer(phi_rating ~ total_direction_changes + task_type + (1|subj_id), data = merged_data_direction, refresh = 0)
...
mean_slope_mdl_5 <- mean(coef_mdl_5$subj_id[2]$total_direction_changes)
...
plot(merged_data_direction$total_direction_changes, merged_data_direction$phi_rating,
     xlab="Total Direction Changes", 
...
colors <- ifelse(merged_data_direction$task_type=="catch", "blue", "red")
plot(merged_data_direction$total_direction_changes, merged_data_direction$phi_rating,
     xlab = "Total Direction Changes", 
     ylab = "Consciousness Rating", 
...
ggplot(data = sims_5_df, aes(x = total_direction_changes)) +
  ...
  annotate("text", x = c(CI_mdl_5[[1]] + 0.1, mean_slope_mdl_5 + 0.1, CI_mdl_5[[2]] - 0.1), y = 1.3, label = c("-2 SE", "Mean", "+2 SE"),
           color = "Black", vjust = 0, size = 3)
```
