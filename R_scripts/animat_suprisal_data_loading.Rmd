...
animating_animat <- function(t, a, r, batch_name) {
...
### Saving plots in a gif_file:
generate_gif_filename <- function(prefix = "animation", extension = "gif") {
  identifier <- paste0(sampled_data$stratum[s], "_",
                       "trial", t, "_", 
                       "agent", a, "_", 
                       "run", r, "_", 
                       "sight", first_sight_event, "_", 
...
avg_phi_data <- ts_tibble %>% summarise(mean(Phi))
...
ggplot(avg_phi_data, aes(x = avg_phi, fill = "red")) +
  geom_density() +
  theme_minimal() +
  ggtitle("Fig. 1: Mean Phi distribution across all runs")
```
